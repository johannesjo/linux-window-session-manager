...
function parseExecutableFileFromWmClassName(wmClassName) {
  ...
  if (executableFileFromMap) {
    return executableFileFromMap;
  } else {
    const splitValues = wmClassName.split('.');
    const fileName = splitValues[0];

    if (isChromeApp(fileName)) {
      console.log(parseChromeAppDesktopFilePath(fileName));

      return parseChromeAppDesktopFilePath(fileName);
    } else {
...
function parseChromeAppDesktopFilePath(fileName) {
  // TODO this does not work, when a user has multiple profiles for chrome
  return fileName.replace('crx_', 'chrome-') + '-Profile_1.desktop';
}