...
module.exports = (passedCFG) => {
  ...
  return {
    ...
    // node x11
    goToFirstWorkspace: ME_goToFirstWorkspace,
    restoreWindowPosition: ME_restoreWindowPosition,
    getWindowGeometry: ME_getWindowGeometry,
    closeWindow: ME_closeWindow,
    goToWorkspace: ME_goToWorkspace,

    // other cmd
    getActiveWindowList: ME_getActiveWindowList,
    getConnectedDisplaysId: ME_getConnectedDisplaysId,
    readAndSetAdditionalMetaDataForWin: ME_readAndSetAdditionalMetaDataForWin,
    findDesktopFile: ME_findDesktopFile,
    startProgram: ME_startProgram,
  };
};
...
function closeWindow(windowId) {
  return x11w.ME_closeWindow(windowId);
}
...
function restoreWindowPosition(win) {
  ...
  return x11w.ME_restoreWindowPosition(win);
}

function getWindowGeometry(winId) {
  return x11w.ME_getWindowGeometry(winId);
}

function goToWorkspace(winId, workSpaceNr) {
  return x11w.ME_goToWorkspace(winId, workSpaceNr);
}

// OTHER CMD
function getConnectedDisplaysId() {
  return otherCmd.ME_getConnectedDisplaysId();
}

function readAndSetAdditionalMetaDataForWin(win) {
  return otherCmd.ME_readAndSetAdditionalMetaDataForWin(win);
}

function findDesktopFile(file) {
  return utilWithCfg.ME_findDesktopFile(file);
}

function startProgram(executableFile, desktopFilePath) {
  console.log('Starting: ' + (executableFile || desktopFilePath));
  return otherCmd.ME_startProgram(executableFile, desktopFilePath);
}

// MIXED
function getActiveWindowList() {
  return new Promise((fulfill, reject) => {
    return otherCmd.ME_getActiveWindowList()
      .then((windowList) => {
        const promises = [];
