...
const DEFAULT_CFG_FILE_PATH = __dirname + '/default-config.json';
...
try {
  ...
  CFG = JSON.parse(fs.readFileSync(DEFAULT_CFG_FILE_PATH, 'utf8'));
  ...
  fs.writeFileSync(CFG_FILE_PATH, JSON.stringify(CFG), 'utf8');
}

// create data store
db = new Store(SESSION_DATA_DIR);
