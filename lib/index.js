...
module.exports = {
  ...
  ME_getConnectedDisplaysId: metaW.ME_getConnectedDisplaysId,
  resetCfg: () => {
  ...
  return new Promise((fulfill, reject) => {
    db.get(sessionToHandle || 'DEFAULT', (err, sessionData) => {
      ...
      closeAllWindowsIfSet(isCloseAllOpenWindows)
        .then(metaW.ME_goToFirstWorkspace)
        .then(metaW.ME_getConnectedDisplaysId)
...
function closeAllWindowsIfSet(isCloseAll) {
  return new Promise((fulfill, reject) => {
    if (isCloseAll) {
      ...
      metaW.ME_getActiveWindowList()
        .then((currentWindowList) => {
          currentWindowList.forEach((win) => {
            metaW.ME_closeWindow(win.windowId);
          });
...
function isAllAppsStarted(savedWindowList, currentWindowList) {
  ...
  savedWindowList.forEach((win) => {
    if (!getMatchingWindowId(win, currentWindowListCopy)) {
      isAllStarted = false;
    } else {
...
function guessFilePath(win, inputHandler) {
  return new Promise((fulfill, reject) => {
    ...
    if (isDesktopFile(win.executableFile)) {
      metaW.ME_findDesktopFile(win.executableFile)
        .then((stdout) => {
...
function startSessionPrograms(windowList, currentWindowList) {
  ...
  windowList.forEach((win) => {
    ...
    if (!isProgramAlreadyRunning(win.wmClassName, currentWindowList, numberOfInstancesOfWin, win.instancesStarted)) {
      promises.push(metaW.ME_startProgram(win.executableFile, win.desktopFilePath));
...
function restoreWindowPositions(savedWindowList) {
  ...
  savedWindowList.forEach((win) => {
    promises.push(() => {
      return metaW.ME_restoreWindowPosition(win);
    });
    promises.push(() => {
      return metaW.ME_goToWorkspace(win.windowId, win.wmCurrentDesktopNr);
    });
  });
