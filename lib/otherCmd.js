...
function readAndSetAdditionalMetaDataForWin(win) {
  return new Promise((fulfill, reject) => {
    ...
    x11(CFG).getWindowInfo(win.windowId);
...
function startProgram(executableFile, desktopFilePath) {
  isDebug() && console.log('DEBUG: startProgram():', executableFile, desktopFilePath);
    ...
    getActiveWindowIds()
      .then((windowIds) => {
        ...
        Promise.all(promises)
          .then(() => {
            isDebug() && console.log('DEBUG: getActiveWindowList():', windowList);
