...
// TODO give an array as response currently it's only a (multi-line) string
function locate(file) {
  return new Promise((fulfill, reject) => {
    exec(CFG.CMD.LOCATE + ' ' + file, (error, stdout, stderr) => {
      if (error || stderr) {
        console.error('locate', file, error || stderr);
        reject(error || stderr);
      } else {
        fulfill(stdout);
      }
            ...
            const filteredWindows =
              windowList
                .filter((win) => {
                  ...
                  !hasWmClassName && console.warn(win.windowId + ' has no wmClassName. Win: ', win);
